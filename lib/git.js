 ...
 * Returns repo object
**/
function prepareRepo (remoteUrl, dir) {
	...
	if (fs.existsSync(repo.dot_git)) {
		...
		rimraf.sync(dir);
		git.clone(remoteUrl, dir, function (err, repo) {
			if ( err ) {
				deferred.reject(err);
			} else {
				deferred.resolve(new Git(repo).status());
...
/*
 * Add files
 * @param files {[String], String} - Array of String paths; or a String path.
**/
